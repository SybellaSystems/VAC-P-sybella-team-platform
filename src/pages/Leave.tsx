...
export function Leave({ user }: { user?: { id?: string; name?: string; role?: string } }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!error && data) setItems(data as LeaveRequest[]);
      setLoading(false);
    };
              ...
              items.map((r) => (
                <div key={r.id} className="bg-white/5 border border-white/10 rounded-3xl p-6">
                  <div className="flex items-start justify-between gap-6">
                    ...
                    <div className="text-white/30 text-xs uppercase font-black tracking-widest">
                      {r.status}
                    </div>
                  </div>
                  {r.reason ? <div className="text-white/60 text-sm mt-4">{r.reason}</div> : null}
                  {r.decided_at ? (
                    <div className="text-white/20 text-xs mt-4">Decided at: {new Date(r.decided_at).toLocaleString()}</div>
                  ) : null}
                </div>
              ))
            )}
