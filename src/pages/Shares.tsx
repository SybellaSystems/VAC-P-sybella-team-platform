...
export function Shares({ user }: { user?: { id?: string; name?: string; role?: UserRole } }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      // Shares are SUPERADMIN-only via RLS (see SQL). Normal users will see empty.
      const { data: s, error: sErr } = await supabase.from('shares').select('*').order('created_at', { ascending: false });
      if (!sErr && s) setShares(s);

      const { data: o, error: oErr } = await supabase
        .from('ownership_records')
        .select('*')
        .order('acquired_at', { ascending: false });
      if (!oErr && o) setOwnership(o);
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-10">
          ...
          <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-white">
            Company Equity Dossier
          </h2>
          <p className="text-white/40 mt-4 max-w-2xl">
            View share classes and ownership allocation.
          </p>
        </header>
