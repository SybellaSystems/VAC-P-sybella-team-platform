        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
              activeTab === 'pending' ? 'bg-primary text-primary-foreground' : 'bg-white border border-input text-foreground hover:bg-muted'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
              activeTab === 'completed' ? 'bg-primary text-primary-foreground' : 'bg-white border border-input text-foreground hover:bg-muted'
            }`}
          >
        ...
        {displayWorkflows.length === 0 ? (
          ...
          <div className="space-y-3">
            {displayWorkflows.map(workflow => {
              // Safe fallback for entity_type
              const rawEntityType = workflow.entity_type || 'other';
              const EntityIcon = entityIcons[rawEntityType] || FileText;
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            ...
                            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${statusCfg.color} flex items-center gap-1`}>
                              <StatusIcon size={9} /> {workflow.status}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="px-4 sm:px-5 pb-4 pt-0 border-t border-border">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-foreground">{workflow.current_step} / {workflow.total_steps}</p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">Status</p>
                          <p className="font-medium text-foreground capitalize">{workflow.status}</p>
                        </div>
                        <div>
